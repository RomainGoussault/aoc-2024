...
def get_antinodes_coords(antenna_0: tuple, antenna_1: tuple) -> list[tuple]:
    i_first = antenna_0[0]
    j_first = antenna_0[1]
    ...
    i_antinode_0 = i_first - (i_second - i_first)
    j_antinode_o = j_first - (j_second - j_first)
    antinode_0_coord = (i_antinode_0, j_antinode_o)

    i_antinode_1 = i_second + (i_second - i_first)
    j_antinode_1 = j_second + (j_second - j_first)
    antinode_1_coord = (i_antinode_1, j_antinode_1)

    return [antinode_0_coord, antinode_1_coord]
    ...
    for antenna_0 in antenna_list:
        for antenna_1 in antenna_list:
            if antenna_0 == antenna_1:
                continue
            antinodes_coords = get_antinodes_coords(antenna_0, antenna_1)
